...
void FeatureTracker::trackFeatures(const cv::Mat &img_1,
  ...
  //
  // ~~~~ begin solution
  cv::Mat img_1_keypoints, img_2_keypoints;
  cv::drawKeypoints(img_1, keypoints_1, img_1_keypoints, cv::Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
  cv::drawKeypoints(img_2, keypoints_2, img_2_keypoints, cv::Scalar::all(-1), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);

  cv::imshow("img_1_keypoints", img_1_keypoints);
  cv::imshow("img_2_keypoints", img_2_keypoints);

  cv::imwrite("img_1_keypoints.png", img_1_keypoints);
  cv::imwrite("img_2_keypoints.png", img_2_keypoints);
  ...
  std::cout << "Descriptor size for img_1: " << descriptors_1.cols << std::endl;//add-1
  describeKeypoints(img_2, &keypoints_2, &descriptors_2);
  std::cout << "Descriptor size for img_2: " << descriptors_2.cols << std::endl;//add-1

  if (matched_kp_1_kp_2) {
    matched_kp_1_kp_2->first = keypoints_1;
    matched_kp_1_kp_2->second = keypoints_2;
  }
  // ~~~~ end solution
  //
  // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
  ...
  for (const auto &m : good_matches) {
    matched_kp_1.push_back(keypoints_1[m.queryIdx]);
    matched_kp_2.push_back(keypoints_2[m.trainIdx]);
  }

  std::vector<unsigned char> inliers;
  inlierMaskComputation(matched_kp_1, matched_kp_2, &inliers);
  ...
  if (!inlier_matches.empty()){
    cv::Mat img_inliers;
    cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, inlier_matches, img_inliers,
                    cv::Scalar(0,255,0), cv::Scalar::all(-1), std::vector<char>(),
                    cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
    cv::imshow("Inlier Matches", img_inliers);
    cv::imwrite("inlier_matches.png", img_inliers);
    cv::waitKey(0);
  }else{
  ...
  //
  // ~~~~ begin solution
  std::vector<DMatch> outlier_matches;
  std::vector<DMatch> inlier_matches_local;
  std::vector<char> char_inlier_mask{inliers.begin(), inliers.end()};

  for (size_t i = 0; i < inliers.size(); ++i) {
    if (char_inlier_mask[i]) {
      inlier_matches_local.push_back(good_matches[i]);
      num_inliers++;
    }else{
  ...
  cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, outlier_matches,
                  img_matches_outliers, cv::Scalar(0, 0, 255), // Red color for outliers
                  cv::Scalar::all(-1), std::vector<char>(),
                  cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);

  cv::Mat img_matches_with_inliers_outliers = img_matches_outliers.clone();
  cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, inlier_matches_local,
                  img_matches_with_inliers_outliers, cv::Scalar(0, 255, 0), // Green color for inliers
                  cv::Scalar::all(-1), std::vector<char>(),
                  cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS | cv::DrawMatchesFlags::DRAW_OVER_OUTIMG);

  // Display and save the visualization
  cv::imshow("Inlier and Outlier Matches", img_matches_with_inliers_outliers);
  cv::imwrite("matches_inliers_outliers.png", img_matches_with_inliers_outliers);
  cv::waitKey(10); // Pause to view the matches
