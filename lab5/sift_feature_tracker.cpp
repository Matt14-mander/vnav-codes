...
void SiftFeatureTracker::describeKeypoints(const cv::Mat& img,
  ...
  //     this function, and you should only need one function call.
  //
  if (!descriptors->empty()) {//add-2
    std::cout << "Descriptor size: " << descriptors->cols << " (should be 128)" << std::endl;
  } else {
...
void SiftFeatureTracker::matchDescriptors(
  ...
  for(const auto& match_pair : *matches){
    if(match_pair.size() == 2){
      const DMatch& best_match = match_pair[0];
      const DMatch& second_best_match = match_pair[1];
