  ...
  case 0: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac) {
        ...
        int num_inliers = extractPose(fivept_nister_essentials, pts1, pts2, camera_params_, R, t);
        ...
        // RANSAC
        int num_iters = 0;
        const float threshold = 1.0 - cos(atan(sqrt(2.0)*0.5/800.0));
        ...
        while(num_iters < max_iters){
          ...
          cv::Mat Rcv, tcv;
          int retval = extractPose(fivept_nister_essentials, corr1, corr2, camera_params_, Rcv, tcv);
          if(retval <= 0){
            num_iters ++; continue;
          }
        ...
        cv::Mat Rcv, tcv;
        extractPose(fivept_nister_essentials, pts1, pts2, camera_params_, Rcv, tcv);
  ...
  case 1: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac) {
        ...
        shared_ptr<CentralRelativePoseSacProblem> solver_ptr(new CentralRelativePoseSacProblem(adapter_mono, 
  ...
  case 2: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      ...
      if (!FLAGS_use_ransac) {
        // Without RANSAC (OPTIONAL)
        // ***************************** begin solution *****************************
        // Without RANSAC (2-point method)
        Eigen::MatrixXd A(2, 3);
        Eigen::VectorXd b(2);
        for (size_t i = 0; i < min_nr_of_correspondences; ++i) {
            Eigen::Vector3d f1 = adapter_mono.getBearingVector1(i);
            Eigen::Vector3d f2 = adapter_mono.getBearingVector2(i);

            A.row(i) = f1.cross(rotation * f2).transpose().block<1, 3>(0, 0);
            b(i) = -f1.dot(rotation * f2);
        }
        ...
        best_transformation.block<3, 3>(0, 0) = rotation;
        ...
        // (TODO) With RANSAC
        // ***************************** begin solution *****************************
        // With RANSAC (2-point method)
        opengv::sac::Ransac<opengv::sac_problems::relative_pose::CentralRelativePoseSacProblem> ransac;

        // Create and initialize the SAC problem
        auto sac_model = std::make_shared<opengv::sac_problems::relative_pose::CentralRelativePoseSacProblem>(
              adapter_mono, opengv::sac_problems::relative_pose::CentralRelativePoseSacProblem::NISTER
        );

        // Assign the SAC model to RANSAC
        ransac.sac_model_ = sac_model;
        ...
        // Retrieve the best transformation from RANSAC
        opengv::transformation_t best_transformation = ransac.model_coefficients_;
  ...
  case 3: {
    ...
    if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac){
        ...
        ransac.threshold_ = 1.0 - cos(atan(sqrt(2.0)*0.5/800.0)); // adjust this <TODO>
        ransac.max_iterations_ = 100; // adjust this <TODO>
        ransac.computeModel(0);
        // return the result
        opengv::transformation_t best_transformation = ransac.model_coefficients_;
        relative_pose_estimate = eigen2Pose(best_transformation);
